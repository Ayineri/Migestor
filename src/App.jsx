...
```
  ...
  <div style={{ background: "#fff", borderBottom: "1.5px solid #E2EEFF", padding: "14px 18px", display: "flex", alignItems: "center", gap: 12 }}>
    ...
    <div>
      <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 16, color: "#1a2e4a", lineHeight: 1.2 }}>
  ...
  <div style={{ background: "#fff", borderTop: "1.5px solid #E2EEFF", padding: "12px 14px 30px", display: "flex", gap: 10, alignItems: "flex-end" }}>
    <textarea
      ...
      rows={2}
      style={{ flex: 1, background: "#F0F6FF", border: "1.5px solid #BFDBFE", borderRadius: 14, color: "#1a2e4a", padding: "11px 14px", fontSize: 15, fontFamily: "'Nunito', sans-serif", fontWeight: 500, resize: "none", outline: "none", lineHeight: 1.5 }}
    />
...
const open = (s = null) => { setChat(s); setChatOpen(true); };

return (
<div style={{ minHeight: “100vh”, background: “#F7FAFF”, fontFamily: “‘Nunito’, sans-serif”, color: “#1a2e4a” }}>
<style>{`@import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,wght@0,600;0,700;0,900;1,600&family=Nunito:wght@400;500;600;700;800&display=swap'); *{box-sizing:border-box;margin:0;padding:0} @keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}} .scard{transition:transform .18s,box-shadow .18s} .scard:hover{transform:translateY(-3px);box-shadow:0 10px 28px rgba(43,127,255,.13)!important} textarea:focus{border-color:#93C5FD!important;background:#fff!important;outline:none} textarea::placeholder{color:#BFDBFE} ::-webkit-scrollbar{width:3px} ::-webkit-scrollbar-thumb{background:#BFDBFE;border-radius:4px}`}</style>
...
```
  ...
  <nav style={{ background: "#fff", borderBottom: "1.5px solid #E2EEFF", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 19, color: "#1a2e4a", letterSpacing: "-0.3px" }}>MiGestor</span>
    </div>
    <button onClick={() => open()} style={{ background: "#2B7FFF", border: "none", color: "#fff", padding: "9px 18px", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Nunito', sans-serif", boxShadow: "0 4px 12px rgba(43,127,255,.3)" }}>
      Pedir ayuda
    </button>
  </nav>
  ...
  <section style={{ padding: "48px 22px 36px", maxWidth: 580, margin: "0 auto", textAlign: "center", opacity: ready ? 1 : 0, transform: ready ? "none" : "translateY(18px)", transition: "all .55s ease" }}>
    ...
    <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(32px, 8vw, 52px)", fontWeight: 900, lineHeight: 1.15, color: "#1a2e4a", marginBottom: 18, letterSpacing: "-0.5px" }}>
    ...
    <button onClick={() => open()} style={{ background: "#2B7FFF", border: "none", color: "#fff", padding: "17px 38px", borderRadius: 16, fontSize: 16, fontWeight: 800, cursor: "pointer", boxShadow: "0 8px 22px rgba(43,127,255,.38)", fontFamily: "'Nunito', sans-serif", letterSpacing: "0.2px" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 11 }}>
      {services.map((s, i) => (
        <button key={s.id} className="scard" onClick={() => open(s)}
          style={{ background: "#fff", border: "1.5px solid #E2EEFF", borderRadius: 18, padding: "18px 14px", textAlign: "left", cursor: "pointer", boxShadow: "0 2px 10px rgba(43,127,255,.06)", opacity: ready ? 1 : 0, transform: ready ? "none" : "translateY(14px)", transition: `opacity .45s ${.08 + i*.06}s ease, transform .45s ${.08 + i*.06}s ease, box-shadow .18s` }}>
          ...
          <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 14, color: "#1a2e4a", marginBottom: 5, lineHeight: 1.3 }}>{s.title}</div>
  ...
  <section style={{ background: "#fff", margin: "0 14px 22px", borderRadius: 22, padding: "26px 20px", maxWidth: 632, marginLeft: "auto", marginRight: "auto", border: "1.5px solid #E2EEFF", boxShadow: "0 2px 14px rgba(43,127,255,.06)" }}>
    <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 20, textAlign: "center", marginBottom: 24, color: "#1a2e4a" }}>Así de fácil</h2>
    {[
      ...
      <div key={i} style={{ display: "flex", gap: 14, marginBottom: i < 2 ? 20 : 0, alignItems: "flex-start" }}>
        ...
        <div style={{ paddingTop: 2 }}>
          <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 15, color: "#2B7FFF", marginBottom: 3 }}>{s.step}</div>
  ...
  <section style={{ padding: "0 14px 110px", maxWidth: 660, margin: "0 auto" }}>
    <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 18, textAlign: "center", marginBottom: 16, color: "#1a2e4a" }}>Quienes ya lo usan</h2>
    {[
      ...
      <div key={i} style={{ background: "#fff", border: "1.5px solid #E2EEFF", borderRadius: 18, padding: "16px", marginBottom: 11, display: "flex", gap: 14, boxShadow: "0 2px 10px rgba(43,127,255,.05)" }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 5 }}>
            <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 14, color: "#1a2e4a" }}>{t.name}</span>
  ...
  <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "10px 14px 26px", background: "linear-gradient(to top, #F7FAFF 65%, transparent)", textAlign: "center" }}>
    <button onClick={() => open()} style={{ background: "#2B7FFF", border: "none", color: "#fff", padding: "16px 28px", borderRadius: 16, fontSize: 15, fontWeight: 800, cursor: "pointer", boxShadow: "0 8px 22px rgba(43,127,255,.38)", width: "100%", maxWidth: 420, fontFamily: "'Nunito', sans-serif" }}>
      🤝 Hablar con mi gestor
    </button>
  </div>
</div>
